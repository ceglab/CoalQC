...
scaf<-"scaffold4"
call_ind <- read.table("../iw_callable_status.bed")
het_ind <- read.table("../ind_iw_hetcount_5k.bed")
scaflist <- read.table("../longest_25_scaf.txt")
scaffolds <- "scaffold1281"
for (i in scaffolds){
scaf <- i
pdf(paste(i,"_int_dist.pdf",sep=""),width=20,height=15)
par(mar=c(4,10,2.5,2.5))
read.table(file="../d.115.RES1386-IW.rg.qf.s100.dc",header=FALSE)->M
M[M$V1==scaf,]-> Ms
plot(c(0:max(Ms$V3)),rep(5,max(Ms$V3)+1),type="n",ylim=c(0,20),main=i,xlab="",ylab="",axes=FALSE,cex.lab=3,col.lab="red")
...
read.table(file="../d.105.RES1386-IW.rg.qf.s100.dc",header=FALSE)->M
M[M$V1==scaf,]-> Ms
rect(Ms$V2,rep(2.1,length(Ms$V2)),Ms$V3,rep(4,length(Ms$V2)),col=colvec[Ms$V4])
read.table(file="../d.85.RES1386-IW.rg.qf.s100.dc",header=FALSE)->M
M[M$V1==scaf,]-> Ms
rect(Ms$V2,rep(4.1,length(Ms$V2)),Ms$V3,rep(6,length(Ms$V2)),col=colvec[Ms$V4])
read.table(file="../d.65.RES1386-IW.rg.qf.s100.dc",header=FALSE)->M
M[M$V1==scaf,]-> Ms
rect(Ms$V2,rep(6.1,length(Ms$V2)),Ms$V3,rep(8,length(Ms$V2)),col=colvec[Ms$V4])
read.table(file="../d.45.RES1386-IW.rg.qf.s100.dc",header=FALSE)->M
M[M$V1==scaf,]-> Ms
...
legend(max(Ms$V3)/20,18.5,legend=seq(0,63,1),ncol = 20,fill=colvec,cex=1.35,bty="n",text.font=2)
text(max(Ms$V3)/2,19,"Atomic Intervals",cex=1.35,font=2)
legend("topright",legend=c(levels(Cs$V4)), fill=callcol,cex=1.25,bty="n",text.font=2)
...
y.label <- c("-t 115", "-t 105", "-t 85", "-t 65", "-t 45", "Callability", "Heterozygosity")
